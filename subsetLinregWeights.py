...
for i in range(22):
	...
	for k in range(features.shape[1]):
		...
		if (min_mse_sofar>min_error):
			...
			r2_sofar = r2
			best_clf_sofar = best_clf
		else:
			break
		# print(min_error)
	list = best_clf_sofar.coef_.tolist()
	weights = np.zeros([features.shape[1]])
	for num in range(len(lowest_mse_feat)):
		weights[lowest_mse_feat[num]] = list[num]
	for val in weights:
		print(val,'\t',end='')
	print()
# exit(1)
